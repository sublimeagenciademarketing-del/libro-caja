...
export default function AdminPage() {
  ...
  async function handleSaveLic() {
    if (!modal) return;
    setSaving(true);
    const { user, fechaVenc, activo } = modal;
  ...
  const counts = {
    ...
    blocked: users.filter(u => u.status === 'blocked' || u.status === 'expiring').length,
  };
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', padding: '20px 16px 40px', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
        {[
          ...
          { key: 'active', label: 'Activos', value: counts.active, color: '#34d399' },
          { key: 'demo', label: 'Demo / Vencidos', value: counts.demo + counts.blocked, color: '#f87171' },
        ].map(s => {
        ...
        {filtered.map(u => (
          <div key={u.user_id} style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${successId === u.email ? 'rgba(52,211,153,0.4)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 16, padding: '14px 16px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>
              ...
              {u.email !== ADMIN_EMAIL && (
                <button
                  onClick={() => setModal({ user: u, fechaVenc: u.fechaVenc || new Date(Date.now() + 365*24*60*60*1000).toISOString().slice(0,10), activo: true })}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 20 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {[{ v: true, label: 'Activa', color: '#34d399' }, { v: false, label: 'Bloqueada', color: '#f87171' }].map(opt => (
                    <button key={String(opt.v)} onClick={() => setModal(m => ({ ...m, activo: opt.v }))}
                      style={{ flex: 1, padding: '10px', borderRadius: 12, border: `1px solid ${modal.activo === opt.v ? opt.color : 'rgba(255,255,255,0.1)'}`, background: modal.activo === opt.v ? `${opt.color}20` : 'transparent', color: modal.activo === opt.v ? opt.color : 'rgba(255,255,255,0.4)', fontFamily: 'inherit', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>
                      {opt.label}
                    </button>
                  ))}
                </div>
