...
// Resumen por cuenta (cuenta doble: "Este mes", compromisos, promedio y monedas
// extra separados por cuenta) + iconito de ayuda en "Compromisos del mes".
export const PILOTO_RESUMEN_CUENTAS = null; // abierto a todos el 21/09 (antes: [ADMIN_EMAIL, zony])
export const puedeResumenCuentas = (email) => enPiloto(PILOTO_RESUMEN_CUENTAS, email);

// Tarjetas: total del período (lo que se paga en el próximo vencimiento),
// elegir en qué período cae cada compra y reacomodar al cambiar el cierre.
export const PILOTO_TARJETA_PERIODO = null; // abierto a todos el 24/09 (antes: [ADMIN_EMAIL, zony])
export const puedeTarjetaPeriodo = (email) => enPiloto(PILOTO_TARJETA_PERIODO, email);

// Resumen: "Dónde estás hoy", una línea por sección con lo que se debe o falta.
export const PILOTO_RADIOGRAFIA = null; // abierto a todos el 24/09 (antes: [ADMIN_EMAIL, zony])
export const puedeRadiografia = (email) => enPiloto(PILOTO_RADIOGRAFIA, email);
